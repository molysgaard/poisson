...
implementations for supercomputers are using SIMD as well as thread level parallelism. This makes
it very hard to implement a true sequential program when using other libraries since these almost always
link to some underlying BLAS or LAPACK routine which is parallelized. The sequential and parallel code
...
SUMMA program and written to disk. Then the SUMMA algorithm would be run and the
...
a parallel fashion, that is, that all the nodes only do initialization for their small part of the
